...
std::size_t RangeMutex<Lock, Guard>::lock(std::size_t begin, std::size_t end) {
  std::size_t lockId = 0;
...
template <class Lock, template <class> class Guard>
void RangeMutex<Lock, Guard>::unlock(std::size_t lockId) {
...
template <class Lock, template <class> class Guard>
std::size_t RangeMutex<Lock, Guard>::try_lock(std::size_t begin,
                                            std::size_t end) {
  const Guard lock_guard(mtx);
  for (auto const &it : rangeMap) {
    std::size_t b = it.second.first;
